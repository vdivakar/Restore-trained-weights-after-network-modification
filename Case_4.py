...
with tf.variable_scope("model_vars"):
    ...
    w_new = tf.get_variable("w_new", [3,3,3,3],initializer=tf.contrib.layers.variance_scaling_initializer(factor=1.0, mode='FAN_AVG', uniform=False), trainable=True)
    ...
    b_new = tf.get_variable("b_new", [3],  initializer=tf.constant_initializer(0), trainable=True)
...
def original_model():
    ...
    act_2   = tf.nn.relu(conv2_b, name="Layer2_act")

    conv3   = tf.nn.conv2d(act_2, w3, strides=[1,1,1,1], padding="SAME", name="Layer3_conv")
    output  = tf.nn.bias_add(conv3, b3, name="output_old")

    conv4   = tf.nn.conv2d(output, w_new, strides=[1,1,1,1], padding="SAME", name="Layer4_conv")
    output_new = tf.nn.bias_add(conv4, b_new, name="output")
...
saver_full = tf.train.Saver() #Load/Save all the variables
saver_list = tf.train.Saver(var_list=[w1,w2,w3,b1,b2,b3]) #Load/Save only the specified variables
...
with tf.Session() as sess:
    ...
    saver_list.restore(sess, "checkpoint/model-original")
    ...
    save_path = saver_full.save(sess, "checkpoint/model-Case_4")
